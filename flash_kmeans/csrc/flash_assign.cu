...
template <int K_SPECIALIZED>
__global__ void flash_assign_kernel_256x128x32_aligned_static_k(
    const half* A,
    const half* B_col_major,
    const float* x_norm,
    const float* c_norm,
    ...
    constexpr int k_tile_count = K_SPECIALIZED / K_TILE;
    const int centroid_tile_count = N / TILE_N;
    int current_stage = 0;
    int next_stage = 1;
    int prefetch_stage = 2;
...
template <int K_SPECIALIZED>
__global__ void flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k(
    const half* A,
    const half* B_col_major,
    const float* x_norm,
    const float* c_norm,
        ...
        switch (K) {
            case 128:
                err = cudaFuncSetAttribute(
                    flash_assign_kernel_256x128x32_aligned_static_k<128>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    static_cast<int>(smem_bytes)
                );
                if (err != cudaSuccess) {
                ...
                flash_assign_kernel_256x128x32_aligned_static_k<128><<<grid, block, smem_bytes, stream>>>(
                    A, B_col_major, x_norm, c_norm, output_ids, output_dists, M, N, K);
                break;
            case 256:
                err = cudaFuncSetAttribute(
                    flash_assign_kernel_256x128x32_aligned_static_k<256>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    static_cast<int>(smem_bytes)
                );
                if (err != cudaSuccess) {
                ...
                flash_assign_kernel_256x128x32_aligned_static_k<256><<<grid, block, smem_bytes, stream>>>(
                    A, B_col_major, x_norm, c_norm, output_ids, output_dists, M, N, K);
                break;
            case 512:
                err = cudaFuncSetAttribute(
                    flash_assign_kernel_256x128x32_aligned_static_k<512>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    static_cast<int>(smem_bytes)
                );
                if (err != cudaSuccess) {
                ...
                flash_assign_kernel_256x128x32_aligned_static_k<512><<<grid, block, smem_bytes, stream>>>(
                    A, B_col_major, x_norm, c_norm, output_ids, output_dists, M, N, K);
                break;
            default:
                err = cudaFuncSetAttribute(
...
cudaError_t launch_flash_assign_kernel_256x128x32_aligned_deferred_reduce(
    ...
    switch (K) {
        case 128:
            err = cudaFuncSetAttribute(
                flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k<128>,
                cudaFuncAttributeMaxDynamicSharedMemorySize,
                static_cast<int>(smem_bytes)
            );
            if (err != cudaSuccess) {
            ...
            flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k<128><<<grid, block, smem_bytes, stream>>>(
                A, B_col_major, x_norm, c_norm, output_ids, output_dists, M, N, K);
            break;
        case 256:
            err = cudaFuncSetAttribute(
                flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k<256>,
                cudaFuncAttributeMaxDynamicSharedMemorySize,
                static_cast<int>(smem_bytes)
            );
            if (err != cudaSuccess) {
            ...
            flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k<256><<<grid, block, smem_bytes, stream>>>(
                A, B_col_major, x_norm, c_norm, output_ids, output_dists, M, N, K);
            break;
        case 512:
            err = cudaFuncSetAttribute(
                flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k<512>,
                cudaFuncAttributeMaxDynamicSharedMemorySize,
                static_cast<int>(smem_bytes)
            );
            if (err != cudaSuccess) {
            ...
            flash_assign_kernel_256x128x32_aligned_deferred_reduce_static_k<512><<<grid, block, smem_bytes, stream>>>(
                A, B_col_major, x_norm, c_norm, output_ids, output_dists, M, N, K);
            break;
        default:
            err = cudaFuncSetAttribute(
