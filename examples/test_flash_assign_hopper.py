...
def _load_extension():
    ...
    csrc = repo_root / "flash_kmeans" / "csrc" / "hopper"
    return load(
        name="flash_assign_hopper_ext",
        sources=[
...
def _run_case(ext, num_points: int, num_centroids: int, dim: int, seed: int) -> None:
    ...
    ids_cuda, dists_cuda, _, _ = ext.flash_assign_hopper_cuda(points, centroids, "hopper_k5_k7_v1")
    ...
    ids_triton = euclid_assign_triton(x, c, x_sq, c_sq=c_sq, use_heuristic=True).squeeze(0)
...
def main():
    parser = argparse.ArgumentParser(description="Compile and correctness-check Hopper flash-assign")
    parser.add_argument(
        "--cases",
        nargs="*",
        default=["256,128,128", "512,256,256", "1024,512,512"],
